          ...
          {[
            ...
            <div key={k.label} className="bg-card p-3 lg:p-4 rounded-xl border border-border animate-fade-in" style={{ animationDelay: `${i * 50}ms` }}>
              ...
              <p className={`text-lg lg:text-xl font-black ${
                (k.label === "Realized Profit" && kpis.realizedProfit < 0) || (k.label === "Net Profit" && (kpis.revenue - kpis.totalCOGS - kpis.totalExpenses) < 0) || k.label === "Payables" ? "text-destructive" : "text-foreground"
              }`}>{k.value}</p>
            </div>
          ))}
        </div>
