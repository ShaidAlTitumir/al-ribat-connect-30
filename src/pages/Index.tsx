      ...
      <div className="p-3 lg:p-8 space-y-4 lg:space-y-6 max-w-7xl mx-auto w-full">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 lg:gap-3">
          {[
            ...
            { label: "BDT Balance", value: fmt(kpis.bdtBalance), icon: "account_balance_wallet", accent: kpis.bdtBalance >= 0 ? "text-teal-600 bg-teal-100 dark:bg-teal-950/40" : "text-destructive bg-destructive/10" },
            { label: "RMB Balance", value: `¥${Math.round(kpis.rmbBalance).toLocaleString("en-IN")}`, icon: "currency_yuan", accent: kpis.rmbBalance >= 0 ? "text-blue-600 bg-blue-100 dark:bg-blue-950/40" : "text-destructive bg-destructive/10" },
            ...
            <div key={k.label} className="bg-card p-3 lg:p-4 rounded-xl border border-border animate-fade-in" style={{ animationDelay: `${i * 50}ms` }}>
              ...
              <p className={`text-lg lg:text-xl font-black ${
                (k.label === "Realized Profit" && kpis.realizedProfit < 0) || (k.label === "BDT Balance" && kpis.bdtBalance < 0) || (k.label === "RMB Balance" && kpis.rmbBalance < 0) || k.label === "Payables" ? "text-destructive" : "text-foreground"
              }`}>{k.value}</p>
            </div>
          ))}
        </div>
